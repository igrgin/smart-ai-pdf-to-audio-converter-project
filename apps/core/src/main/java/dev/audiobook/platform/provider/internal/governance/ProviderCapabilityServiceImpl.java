package dev.audiobook.platform.provider.internal.governance;
...
public class ProviderCapabilityServiceImpl implements ProviderCapabilityService {
    ...
    public CapabilityProfile qualified(String profileVersion, ServiceKind service, InputKind input) {
        if (profileVersion == null || profileVersion.isBlank() || service == null || input == null) {
            throw new IllegalArgumentException("A complete provider capability request is required");
        }
        List<StoredProfile> profiles = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> new StoredProfile(
