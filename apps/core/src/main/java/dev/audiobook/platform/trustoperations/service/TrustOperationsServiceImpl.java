package dev.audiobook.platform.trustoperations.service;

import dev.audiobook.platform.identifier.PlatformIdentifierGenerator;
import dev.audiobook.platform.trustoperations.*;
import dev.audiobook.platform.trustoperations.audit.TrustOperationsAuditJournal;
...
public class TrustOperationsServiceImpl implements TrustOperationsService {
    ...
    public OperationsCase openCase(OpenCaseRequest request) {
        ...
        return new OperationsCase(
                caseId,
                request.type(),
                opaqueResourceReference,
                request.resourceKind(),
                request.restrictionCode(),
                request.consequenceCode(),
                request.deadline(),
                request.safetyPriority(),
                request.urgency(),
    ...
    public ListenerAccessSummary listenerAccess(UUID listenerId) {
        List<DelegatedAccessGrant> grants =
                jdbcTemplate.query(
                        grantSelect()
                                + " WHERE g.listener_id = ? ORDER BY g.valid_from DESC, g.grant_id",
    ...
    public EmergencyAccessGrant grantEmergencyAccess(GrantEmergencyAccessCommand command) {
        ...
        policy.requireEmergencyAccess(
                command,
                operationsCase.type().requiredRole(),
                operationsCase.allowedActions(),
                now);
        UUID grantId = identifierGenerator.generate();
        UUID notificationId =
                operationsCase.listenerId() == null ? null : identifierGenerator.generate();
        Instant reviewDueAt = policy.emergencyReviewDueAt(now);
        if (notificationId != null) {
            auditJournal.notifyListener(
                    new NotificationCommand(
                            notificationId,
                            operationsCase.listenerId(),
                            command.caseId(),
                            grantId,
    ...
    public EmergencyAccessGrant reviewEmergencyAccess(ReviewEmergencyAccessCommand command) {
        ...
        if (replay != null) {
            if (!replay.grantId().equals(command.grantId())
                    || replay.reviewOutcome() != command.outcome()) {
                throw new TrustOperationsConflictException();
            }
            return emergencyGrant(replay, false);
        }
    ...
    public PrivilegedActionResult performPrivilegedAction(PrivilegedActionCommand command) {
        ...
        AccessAuthority authority =
                activeAuthority(command.staff().staffId(), command.caseId(), command.action());
        if (authority == null) {
            throw new TrustOperationsAccessDeniedException();
        }
        ...
        if (replay != null) {
            ...
            return new PrivilegedActionResult(
                    new AuthorizedResource(
                            operationsCase.resourceKind(), operationsCase.resourceId()),
                    replay.event());
        }
        ...
        Instant now = identityClock.instant();
        UUID notificationId =
                authority.listenerId() == null ? null : identifierGenerator.generate();
        if (notificationId != null) {
            auditJournal.notifyListener(
                    new NotificationCommand(
                            notificationId,
                            authority.listenerId(),
                            command.caseId(),
                            authority.grantId(),
    ...
    private AccessAuthority activeAuthority(UUID staffId, UUID caseId, PrivilegedAction action) {
        ...
        List<AccessAuthority> delegated =
                jdbcTemplate.query(
                        """
        ...
        List<AccessAuthority> emergency =
                jdbcTemplate.query(
                        """
                        SELECT g.grant_id, g.listener_id, g.purpose_code
                ...
                jdbcTemplate.query(
                        ...
                        (resultSet, row) ->
                                new CaseRow(
                                        ...
                                        resultSet.getObject(
                                                "opaque_resource_reference", UUID.class),
                                        resultSet.getString("restriction_code"),
                                        resultSet.getString("consequence_code"),
                                        resultSet.getTimestamp("deadline").toInstant(),
                                        resultSet.getInt("safety_priority"),
    ...
    private GrantRow singleGrant(String sql, Object argument) {
        List<GrantRow> rows =
                jdbcTemplate.query(sql, (resultSet, row) -> grantRow(resultSet), argument);
        return rows.isEmpty() ? null : rows.getFirst();
    }
    ...
    private EmergencyRow singleEmergency(String sql, Object argument) {
        List<EmergencyRow> rows =
                jdbcTemplate.query(sql, (resultSet, row) -> emergencyRow(resultSet), argument);
        return rows.isEmpty() ? null : rows.getFirst();
    }
    ...
    private static EmergencyRow emergencyRow(java.sql.ResultSet resultSet)
