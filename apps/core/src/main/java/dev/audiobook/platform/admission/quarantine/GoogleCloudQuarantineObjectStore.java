package dev.audiobook.platform.admission.quarantine;
...
public class GoogleCloudQuarantineObjectStore implements QuarantineObjectStore {
    ...
    @Override
    public StoredObject append(UUID objectId, long expectedOffset, byte[] bytes, boolean complete)
            throws IOException {
            ...
            if (completed != null) {
                if (completed.getSize() != nextOffset) {
                    throw new IllegalStateException(
                            "Completed quarantine object has an unexpected length");
                }
        ...
        if (existing == null) {
            try {
                ...
                if (exception.getCode() != 412
                        || !sameBytes(storage.get(bucket, chunkName), bytes)) {
                    throw new IOException("Unable to append the quarantine upload", exception);
                }
            }
        } else if (!sameBytes(existing, bytes)) {
            throw new IllegalStateException(
                    "Quarantine object offset was already used by different bytes");
        }
        ...
        Blob composed =
                storage.compose(
                        Storage.ComposeRequest.newBuilder()
                                .setTarget(
                                        BlobInfo.newBuilder(bucket, sourceKey(objectId))
                                                .setContentType("application/octet-stream")
                                                .build())
    ...
    public void delete(UUID objectId) throws IOException {
        List<BlobId> objects =
                StreamSupport.stream(
                                storage.list(
                                                bucket,
                                                Storage.BlobListOption.prefix(rootPrefix(objectId)))
                                        .iterateAll()
                                        .spliterator(),
                                false)
                        .map(Blob::getBlobId)
