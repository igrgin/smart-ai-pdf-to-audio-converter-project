...
final class LocalOfflineSignatureBoundary implements OfflineSignatureBoundary {
    ...
    LocalOfflineSignatureBoundary(OfflineAccessProperties properties) {
        if (properties.signingPrivateKey() == null
                || properties.signingPrivateKey().isBlank()
                || properties.signingPublicKey() == null
                || properties.signingPublicKey().isBlank()) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Local Offline Copy signing key pair is invalid", exception);
        }
    }
    ...
    public byte[] sign(byte[] payload) {
        try {
            ...
            throw new IllegalStateException(
                    "Offline Copy authorization could not be signed", exception);
        }
    }

    private void verifyKeyPair() throws Exception {
        byte[] probe =
                "folio-offline-signing-key-check".getBytes(java.nio.charset.StandardCharsets.UTF_8);
