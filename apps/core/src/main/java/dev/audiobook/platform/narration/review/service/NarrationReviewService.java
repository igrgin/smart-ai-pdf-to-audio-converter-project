...
public interface NarrationReviewService {
    ...
    record SectionDecision(
        ...
        public SectionDecision {
            sourceChapterOrdinals =
                    sourceChapterOrdinals == null ? List.of() : List.copyOf(sourceChapterOrdinals);
            reviewItems = reviewItems == null ? List.of() : List.copyOf(reviewItems);
        }
    }

    record ReviewItemDecision(
            int sourceChapterOrdinal, int ordinal, Treatment treatment, String narrationSnippet) {}

    record ReviewResult(
            UUID decisionId, ReviewAction action, long conversionVersion, boolean replayed) {}
