...
public class ConversionWorkflowServiceImpl implements ConversionWorkflowService {
    ...
    public DeliveryDecision claimDelivery(WorkDelivery delivery) {
        ...
        if (replay != null) {
            return new DeliveryDecision(DeliveryDisposition.DUPLICATE, replay.stageRunId(), "MESSAGE_REPLAYED");
        }
        if (delivery.schemaVersion() != CURRENT_SCHEMA_VERSION) {
            return recordDelivery(delivery, DeliveryDisposition.DEAD_LETTERED, null, "UNKNOWN_SCHEMA");
        }

        ConversionPosition conversion = lockConversion(delivery.conversionId());
        if (conversion == null) {
            return recordDelivery(delivery, DeliveryDisposition.DEAD_LETTERED, null, "UNKNOWN_CONVERSION");
        }
        ...
        if (conversion.version() != delivery.expectedConversionVersion()) {
            return recordDelivery(delivery, DeliveryDisposition.STALE, null, "CONVERSION_VERSION_MISMATCH");
        }

        StageView stage = lockedStage(delivery.conversionId(), delivery.stage());
        if (stage == null) {
            return recordDelivery(delivery, DeliveryDisposition.DEAD_LETTERED, null, "UNKNOWN_STAGE_RUN");
        }
        Instant now = identityClock.instant();
        boolean expiredLease = stage.state() == StageState.CLAIMED
                && stage.leaseExpiresAt() != null
                && !stage.leaseExpiresAt().isAfter(now);
        if (stage.attemptCount() >= stage.maximumAttempts()) {
            return recordDelivery(delivery, DeliveryDisposition.REJECTED, stage.stageRunId(), "ATTEMPTS_EXHAUSTED");
        }
        if (stage.state() != StageState.READY && !expiredLease) {
            return recordDelivery(delivery, DeliveryDisposition.REJECTED, stage.stageRunId(), "STAGE_NOT_CLAIMABLE");
        }
    ...
    public StageView repairStage(
        ...
        int inserted = jdbcTemplate.update(
                ...
                timestamp(identityClock.instant()));
        if (inserted == 0) {
            throw new IllegalArgumentException("Idempotency-Key was already used for another repair");
        }
        if (current == null
    ...
    public StageView resume(ResumeCommand command) {
        ...
        if (replay != null) {
            if (!replay.matches(command)) {
                throw new IllegalArgumentException("Idempotency-Key was already used for another resume");
            }
        ...
        if (!eligibility.eligible()) {
            throw new IllegalStateException("Conversion resume policy rejected: " + eligibility.denialReason());
        }
        if (requiresRecipe(pause.safeResumeStage())) {
            narrationSelectionService.authorizeGeneration(command.listenerId(), command.conversionId());
        }
    ...
    @Override
    @Transactional
    public CancellationResult cancel(CancellationCommand command) {
        validate(command);
        CancellationOperation replay = cancellationOperation(command.idempotencyKey());
        if (replay != null) {
            if (!replay.matches(command)) {
                throw new IllegalArgumentException("Idempotency-Key was already used for another cancellation");
            }
            return cancellationResult(command.listenerId(), command.conversionId());
        }
        OwnedConversion conversion = lockOwnedConversion(command.listenerId(), command.conversionId());
        if (conversion == null
    ...
    public CancellationResult failTerminal(TerminalFailureCommand command) {
        ...
        if (replay != null) {
            if (!replay.matches(command)) {
                throw new IllegalArgumentException("Idempotency-Key was already used for another terminal failure");
            }
            return cancellationResult(command.listenerId(), command.conversionId());
        }
        OwnedConversion conversion = lockOwnedConversion(command.listenerId(), command.conversionId());
    ...
    public StageView stage(UUID listenerId, UUID conversionId, Stage stage) {
        ...
        Objects.requireNonNull(conversionId, "conversionId");
        Objects.requireNonNull(stage, "stage");
        List<StageView> matches = jdbcTemplate.query(
                """
    ...
    private StageLease lockedStageLease(UUID conversionId, Stage stage) {
        List<StageLease> matches = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> {
                    OffsetDateTime expiresAt = resultSet.getObject("lease_expires_at", OffsetDateTime.class);
    ...
    private static StageView stageView(java.sql.ResultSet resultSet) throws java.sql.SQLException {
        OffsetDateTime leaseExpiresAt = resultSet.getObject("lease_expires_at", OffsetDateTime.class);
    ...
    private ConversionPosition lockConversion(UUID conversionId) {
        List<ConversionPosition> matches = jdbcTemplate.query(
                """
                SELECT state, version FROM workflow.audiobook_conversion
                WHERE conversion_id = ? FOR UPDATE
                """,
                (resultSet, row) -> new ConversionPosition(
    ...
    private ConversionPause pausedConversion(UUID listenerId, UUID conversionId, boolean lock) {
        List<ConversionPause> matches = jdbcTemplate.query(
                ("""
                ...
                """ + (lock ? " FOR UPDATE" : "")),
                (resultSet, row) -> {
                    OffsetDateTime deadline = resultSet.getObject("pause_deadline", OffsetDateTime.class);
                    return new ConversionPause(
                            resultSet.getLong("version"),
                            resultSet.getString("reason_code"),
                            ResponsibleParty.valueOf(resultSet.getString("pause_responsible_party")),
    ...
    private ResumeOperation resumeOperation(String operationKey) {
        List<ResumeOperation> matches = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> new ResumeOperation(
                        resultSet.getObject("listener_id", UUID.class),
    ...
    private OwnedConversion lockOwnedConversion(UUID listenerId, UUID conversionId) {
        List<OwnedConversion> matches = jdbcTemplate.query(
                """
                SELECT state, version FROM workflow.audiobook_conversion
                WHERE listener_id = ? AND conversion_id = ? FOR UPDATE
                """,
                (resultSet, row) -> new OwnedConversion(
                        resultSet.getString("state"), resultSet.getLong("version")),
                listenerId,
                conversionId);
        return matches.isEmpty() ? null : matches.getFirst();
    ...
    private UUID reservationId(UUID listenerId, UUID conversionId) {
        List<UUID> matches = jdbcTemplate.query(
                ...
                """,
                (resultSet, row) -> resultSet.getObject("entitlement_reservation_id", UUID.class),
                listenerId,
                conversionId);
        if (matches.isEmpty()) {
    ...
    private CancellationOperation cancellationOperation(String operationKey) {
        List<CancellationOperation> matches = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> new CancellationOperation(
                        resultSet.getObject("listener_id", UUID.class),
                        resultSet.getObject("conversion_id", UUID.class),
                        resultSet.getLong("expected_conversion_version"),
    ...
    private CancellationResult cancellationResult(UUID listenerId, UUID conversionId) {
        return jdbcTemplate.queryForObject(
                ...
                """,
                (resultSet, row) -> new CancellationResult(
                        resultSet.getObject("conversion_id", UUID.class),
                        AudiobookConversionService.ConversionState.valueOf(resultSet.getString("state")),
                        resultSet.getString("reason_code"),
                        resultSet.getLong("version")),
                listenerId,
                conversionId);
    ...
    private DeliveryDecision existingDelivery(UUID messageId) {
        List<DeliveryDecision> matches = jdbcTemplate.query(
                """
                SELECT disposition, stage_run_id, reason_code
                FROM workflow.conversion_message_inbox WHERE message_id = ?
                """,
                (resultSet, row) -> new DeliveryDecision(
                        DeliveryDisposition.valueOf(resultSet.getString("disposition")),
                        resultSet.getObject("stage_run_id", UUID.class),
                        resultSet.getString("reason_code")),
                messageId);
        return matches.isEmpty() ? null : matches.getFirst();
    ...
    private static AcceptedResult acceptedResult(java.sql.ResultSet resultSet) throws java.sql.SQLException {
    ...
    private static void validate(WorkDelivery delivery) {
        ...
        if (delivery.workerId() == null || delivery.workerId().isBlank() || delivery.workerId().length() > 160) {
            throw new IllegalArgumentException("workerId is required and must be at most 160 characters");
        }
    ...
    private static void validate(StageResult result) {
        ...
        if (result.resultReference() == null
            ...
            throw new IllegalArgumentException("resultReference is required and must be at most 300 characters");
        }
    ...
    private static void validate(StageCheckpoint checkpoint) {
        ...
        if (checkpoint.checkpointDigest() == null
            ...
            throw new IllegalArgumentException("checkpointDigest must be a lowercase SHA-256 digest");
        }
    }
    ...
    private void validate(PauseCommand command) {
        ...
        if (command.reasonCode() == null || command.reasonCode().isBlank() || command.reasonCode().length() > 64) {
            throw new IllegalArgumentException("pause reason is required and must be at most 64 characters");
        }
    ...
    private static void validate(CancellationCommand command) {
        Objects.requireNonNull(command, "command");
        Objects.requireNonNull(command.listenerId(), "listenerId");
        Objects.requireNonNull(command.conversionId(), "conversionId");
        requireOperationKey(command.idempotencyKey());
        ...
        if (command.requestReason() == null
                || command.requestReason().isBlank()
                || command.requestReason().length() > 200) {
            throw new IllegalArgumentException("requestReason is required and must be at most 200 characters");
        }
    }
    ...
    private static void validate(ProviderCost command) {
        ...
        if (command.evidenceReference() == null
            ...
            throw new IllegalArgumentException("evidenceReference is required and must be at most 300 characters");
        }
    }
    ...
    private static void requireOperationKey(String operationKey) {
        if (operationKey == null || operationKey.isBlank() || operationKey.length() > 200) {
            throw new IllegalArgumentException("operation key is required and must be at most 200 characters");
        }
    }
    ...
    private record CancellationOperation(
        ...
        boolean matches(CancellationCommand command) {
            return listenerId.equals(command.listenerId())
                    && conversionId.equals(command.conversionId())
                    && expectedConversionVersion == command.expectedConversionVersion()
                    && incurredProviderCostMicros == command.incurredProviderCostMicros()
                    && requestReason.equals(command.requestReason());
        }
    }
