...
public class IdentitySessionController {
    ...
    public ResponseEntity<SessionView> session(Authentication authentication, CsrfToken csrfToken) {
        ListenerView listener =
                principal(authentication) == null
                        ? null
                        : ListenerView.from(principal(authentication));
    ...
    public ResponseEntity<Map<String, Object>> providers() {
        List<ProviderView> providers =
                Arrays.stream(SignInProvider.values()).map(ProviderView::from).toList();
        return ResponseEntity.ok()
                .cacheControl(CacheControl.noStore())
    ...
    public ResponseEntity<?> link(
            ...
            return ResponseEntity.status(HttpStatus.PRECONDITION_REQUIRED)
                    .body(
                            Map.of(
                                    ...
                                    true,
                                    "authorizationPath",
                                    authorizationPath(principal.currentProvider())));
        }
        ...
        return ResponseEntity.accepted()
                .body(Map.of("authorizationPath", authorizationPath(requestedProvider)));
    }
    ...
    public ResponseEntity<Void> recovery(HttpServletRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.SEE_OTHER)
                .location(properties.recoveryUri())
                .build();
    }
    ...
    public record SessionView(
            boolean authenticated, ListenerView listener, StaffView staff, CsrfView csrf) {}

    public record ListenerView(
            String displayName, String contactEmail, List<String> signInMethods) {
    ...
    public record ProviderView(String id, String label, String authorizationPath) {
        static ProviderView from(SignInProvider provider) {
            ...
            return new ProviderView(
                    id, label, IdentitySessionController.authorizationPath(provider));
        }
    }
}
