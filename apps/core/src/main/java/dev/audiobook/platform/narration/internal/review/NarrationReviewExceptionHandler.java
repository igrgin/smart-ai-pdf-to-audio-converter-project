...
public class NarrationReviewExceptionHandler {
    ...
    @ExceptionHandler({IllegalArgumentException.class, HttpMessageNotReadableException.class, MissingRequestHeaderException.class})
    ProblemDetail invalidRequest(Exception exception) {
        return problem(
                HttpStatus.BAD_REQUEST,
                "INVALID_NARRATION_REVIEW",
                "Invalid Narration Review",
                "Submit only bounded structural decisions and Narration Review Item treatment or snippet changes.");
    }
    ...
    ProblemDetail rejected(NarrationReviewRejectedException exception) {
        HttpStatus status = switch (exception.reason()) {
            ...
            case CONVERSION_VERSION_MISMATCH, IDEMPOTENCY_KEY_REUSED, REVIEW_NOT_AVAILABLE -> HttpStatus.CONFLICT;
            case INVALID_REVIEW -> HttpStatus.BAD_REQUEST;
            case WORKING_ASSET_UNAVAILABLE -> HttpStatus.SERVICE_UNAVAILABLE;
        };
        String detail = switch (exception.reason()) {
            case CONVERSION_VERSION_MISMATCH ->
                    "The Narration Review changed after it was loaded. Reload the latest review and try again.";
            case IDEMPOTENCY_KEY_REUSED ->
                    "This review operation key was already used for different review decisions.";
            case CONVERSION_UNAVAILABLE -> "The Audiobook Conversion is unavailable.";
            case REVIEW_NOT_AVAILABLE -> "This Narration Review is no longer available for changes.";
            case INVALID_REVIEW ->
                    "Submit every source section and review item exactly once using only supported decisions.";
