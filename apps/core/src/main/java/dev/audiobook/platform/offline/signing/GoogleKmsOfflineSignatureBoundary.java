...
import dev.audiobook.platform.offline.authorization.OfflineAccessProperties;
...
final class GoogleKmsOfflineSignatureBoundary implements OfflineSignatureBoundary {
    ...
    GoogleKmsOfflineSignatureBoundary(OfflineAccessProperties properties) throws IOException {
        if (properties.signingKeyVersion() == null
            ...
            throw new IllegalArgumentException(
                    "Cloud KMS Offline Copy signing key version is invalid");
        }
    ...
    public byte[] sign(byte[] payload) {
        try {
            ...
            if (!response.path("verifiedDigestCrc32c").asBoolean()
                    ...
                    || crc32c(derSignature)
                            != Long.parseLong(requiredText(response, "signatureCrc32c"))) {
                throw new IllegalStateException(
                        "Cloud KMS Offline Copy signature integrity check failed");
            }
            return derToP1363(derSignature);
        } catch (Exception exception) {
            throw new IllegalStateException(
                    "Cloud KMS could not sign Offline Copy authorization", exception);
        }
    }
    ...
    private String fetchPublicKey() {
        try {
            JsonNode response =
                    request(
                            URI.create(KMS_ENDPOINT + signingKeyVersion + "/publicKey"),
                            "GET",
                            null);
            if (!"EC_SIGN_P256_SHA256".equals(requiredText(response, "algorithm"))) {
                throw new IllegalStateException(
                        "Cloud KMS Offline Copy key must use EC_SIGN_P256_SHA256");
            }
            ...
            if (!signingKeyVersion.equals(requiredText(response, "name"))
                ...
                throw new IllegalStateException(
                        "Cloud KMS Offline Copy public-key integrity check failed");
            }
            ...
            throw new IllegalStateException(
                    "Cloud KMS Offline Copy public key is unavailable", exception);
        }
    }
    ...
    private JsonNode request(URI uri, String method, String body) throws Exception {
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            throw new IllegalStateException(
                    "Cloud KMS request failed with status " + response.statusCode());
        }
    ...
    private static String requiredText(JsonNode response, String field) {
        ...
        if (value.isBlank())
            throw new IllegalStateException("Cloud KMS response is missing " + field);
        return value;
    }
    ...
    private static void copyInteger(byte[] integer, byte[] destination, int offset) {
        ...
        if (length <= 0 || length > 32)
            throw new IllegalArgumentException("ECDSA integer is invalid");
