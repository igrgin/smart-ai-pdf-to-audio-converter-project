...
public class ProviderAnalysisServiceImpl implements ProviderAnalysisService {
    ...
    public AnalysisOutcome analyze(AnalysisCommand command) {
        ...
        ProviderAnalysisAdapter adapter =
                adapters.stream()
                        .filter(candidate -> capability.provider().equals(candidate.provider()))
                        .findFirst()
                        .orElseThrow(
                                () -> rejected(ProviderAnalysisException.Code.ADAPTER_UNAVAILABLE));
    ...
    private static void validate(AnalysisCommand command) {
        if (command == null
                ...
                || !command.outputSchema()
                        .fields()
                        .keySet()
                        .containsAll(command.outputSchema().requiredFields())) {
                    ...
                    .forEachRemaining(
                            field -> {
                                if (!schema.fields().containsKey(field)) {
                                    throw rejected(
                                            ProviderAnalysisException.Code.INVALID_SCHEMA_OUTCOME);
                                }
                            });
        }
