...
public class ConversionEntitlementServiceImpl implements ConversionEntitlementService {
    ...
    @Override
    @Transactional
    public FreeGrant approveFreeGrant(
            UUID listenerId, String approvalReference, String idempotencyKey) {
        ...
        return new FreeGrant(
                grantId, properties.freeGrantCharacters(), validFrom, validUntil, true);
    }
    ...
    private DemonstrationSubscriptionStatus subscriptionStatus(EntitlementGrant grant) {
        ...
        return jdbcTemplate.query(
                ...
                resultSet ->
                        resultSet.next()
                                ? DemonstrationSubscriptionStatus.valueOf(
    ...
    public AdmissionDecision authorizeSpeech(AdmissionRequest request) {
        ...
        if (spendExposure(null, null) + admission.costMicros()
                > properties.globalSpendCeilingMicros()) {
    ...
    public Settlement settle(SettlementRequest request) {
        ...
        if (reservation == null
                || reservation.reservedCharacters() <= 0
        ...
        Instant now = clock.instant();
        long characterRelease =
                now.isBefore(reservation.grantValidUntil())
                                && !isGrantRevoked(reservation.grantId())
        ...
        jdbcTemplate.update(
                ...
                databaseTime(now));
        saveOperation(
                operationKey, "SETTLEMENT", fingerprint, "SETTLED", request.reservationId(), now);
        audit(
                reservation.listenerId(),
                reservation.conversionId(),
                request.reservationId(),
    ...
    public ProviderSpend providerSpend(String provider) {
        String normalizedProvider =
                requiredReference(provider, "provider").toLowerCase(Locale.ROOT);
        return jdbcTemplate.queryForObject(
                ...
                (resultSet, rowNumber) ->
                        new ProviderSpend(
                                resultSet.getLong("reserved_micros"),
                                resultSet.getLong("committed_micros")),
                normalizedProvider);
    }
    ...
    public Correction correctCharacters(CorrectionRequest request) {
        ...
        String operationKey = requiredReference(request.idempotencyKey(), "idempotencyKey");
        String fingerprint =
                fingerprint(request.listenerId(), request.availableCharacterDelta(), evidence);
        lockEntitlementState();
        ...
        if (current.availableCharacters() + request.availableCharacterDelta() < 0) {
            throw new IllegalArgumentException(
                    "Correction cannot make available characters negative");
        }
        if (current.availableCharacters() + request.availableCharacterDelta()
    ...
    @Override
    @Transactional
    public Expiry expireFreeGrant(
            UUID listenerId, String evidenceReference, String idempotencyKey) {
        Objects.requireNonNull(listenerId, "listenerId");
    ...
    private AdmissionDecision deny(
            ValidatedAdmission admission, AdmissionDenial denial, Instant now) {
        saveOperation(
                admission.operationKey(),
                "ADMISSION",
                admission.fingerprint(),
    ...
    private AdmissionDecision replayAdmission(StoredOperation operation) {
        ...
        return new AdmissionDecision(
                false, null, AdmissionDenial.valueOf(operation.outcome()), true);
    }
    ...
    private ValidatedAdmission validate(AdmissionRequest request) {
        ...
        String provider =
                requiredReference(request.provider(), "provider").toLowerCase(Locale.ROOT);
        String recipe =
                requiredReference(request.generationRecipeReference(), "generationRecipeReference");
                ...
                resultSet ->
                        resultSet.next()
                                ? new FreeGrant(
                                        ...
                                        resultSet
                                                .getObject("valid_from", OffsetDateTime.class)
                                                .toInstant(),
                                        resultSet
                                                .getObject("valid_until", OffsetDateTime.class)
                                                .toInstant(),
                                        false)
                                : null,
                value);
    ...
    private static EntitlementGrant entitlementGrant(java.sql.ResultSet resultSet)
    ...
    private boolean isGrantRevoked(UUID grantId) {
        Integer count =
                jdbcTemplate.queryForObject(
                        "SELECT count(*) FROM demonstration_subscription_grant_adjustment WHERE"
                                + " grant_id = ?",
                        Integer.class,
                        grantId);
    ...
    private record StoredOperation(
            String type, String fingerprint, String outcome, UUID relatedId) {}
