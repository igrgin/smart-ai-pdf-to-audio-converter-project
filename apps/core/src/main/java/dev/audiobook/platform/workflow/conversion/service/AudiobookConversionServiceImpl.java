package dev.audiobook.platform.workflow.conversion.service;
...
public class AudiobookConversionServiceImpl implements AudiobookConversionService {
    ...
    private final ConversionLifecycleService conversionLifecycleService;
    ...
    public void scheduleNarrationPlan(UUID listenerId, UUID conversionId, UUID submissionId) {
        ...
        jdbcTemplate.update(
                ...
                now);
        conversionWorkflowService.scheduleStage(
                listenerId, conversionId, ConversionWorkflowService.Stage.NARRATION_ANALYSIS, 4);
    }
                ...
                jdbcTemplate.update(
                        ...
                        WHERE c.state = 'PREPARING'
                          AND EXISTS (
                              ...
                              WHERE work.conversion_id = c.conversion_id AND work.state = 'SUCCEEDED'
                          )
                          AND EXISTS (
                              SELECT 1
                              FROM workflow.conversion_accepted_result accepted
                              JOIN narration.narration_plan plan
                                ON plan.conversion_id = accepted.conversion_id
                               AND plan.working_asset_ref = accepted.result_reference
                               AND plan.asset_sha256 = accepted.result_sha256
                              WHERE accepted.conversion_id = c.conversion_id
                                AND accepted.stage = 'NARRATION_ANALYSIS'
                          )
                        """);
        int exhausted =
                jdbcTemplate.update(
    ...
    public AudiobookConversion conversion(UUID listenerId, UUID conversionId) {
        Objects.requireNonNull(listenerId, "listenerId");
        Objects.requireNonNull(conversionId, "conversionId");
        List<AudiobookConversion> matches =
                jdbcTemplate.query(
                        """
                        SELECT c.conversion_id, c.state, c.reason_code, c.version,
                               w.resume_from_page, w.listener_guidance
    ...
    private static AudiobookConversion conversion(java.sql.ResultSet resultSet)
        ...
        return new AudiobookConversion(
                ...
                state == ConversionState.AWAITING_REVIEW
                                && "NARRATION_REVIEW_AVAILABLE"
                                        .equals(resultSet.getString("reason_code"))
                        ? List.of(
                                AllowedAction.REVIEW_NARRATION_PLAN,
                                AllowedAction.ACCEPT_RECOMMENDATIONS)
                ...
                resumeFromPage == null
                        ? null
                        : new RecoveryDetails(
                                resumeFromPage, resultSet.getString("listener_guidance")));
    }
    ...
    public AudiobookConversion resumeNarrationPlan(
        ...
        if (accepted == 0) {
            ...
            throw new IllegalArgumentException(
                    "Idempotency-Key was already used for another recovery action");
        }
        conversionLifecycleService.resume(
        ...
        UUID workId =
                jdbcTemplate.queryForObject(
                        "SELECT work_id FROM workflow.narration_plan_work WHERE conversion_id = ?"
                                + " AND listener_id = ?",
                        UUID.class,
                        conversionId,
                        listenerId);
        ...
        if (started == 0) {
            ...
            if (!ConversionState.GENERATING.name().equals(state)) {
                throw new IllegalStateException(
                        "Audiobook Conversion cannot begin speech generation");
            }
        }
        conversionWorkflowService.scheduleStage(
                listenerId, conversionId, ConversionWorkflowService.Stage.SPEECH, 4);
