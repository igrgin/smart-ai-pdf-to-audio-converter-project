...
import dev.audiobook.platform.workflow.conversion.dto.ConversionProgress;
...
public class AudiobookConversionController {
    ...
    public ResponseEntity<ConversionProgress> conversion(
        ...
        NarrationPlanService.PlanView plan =
                conversion.state() == AudiobookConversionService.ConversionState.AWAITING_REVIEW
    ...
    public ResponseEntity<ConversionProgress> cancel(
            ...
            @RequestHeader("If-Match") String ifMatch) {
        long expectedVersion = expectedVersion(ifMatch);
        ConversionLifecycleService.CancellationResult cancellation =
    ...
    private static long expectedVersion(String ifMatch) {
        ...
        if (value.length() < 3
            ...
            throw new IllegalArgumentException(
                    "If-Match must contain the quoted conversion version");
        }
    ...
    private static boolean matches(String ifNoneMatch, String entityTag) {
        ...
        for (String candidate : ifNoneMatch.split(",")) {
            ...
            if (normalized.equals("*")
                    || normalized.equals(entityTag)
                    || normalized.equals("W/" + entityTag)) {
                return true;
            }
        }
        return false;
