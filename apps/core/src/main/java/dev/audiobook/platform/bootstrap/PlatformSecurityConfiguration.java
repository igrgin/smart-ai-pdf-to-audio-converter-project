...
import dev.audiobook.platform.entitlement.subscription.stripe.StripeDemonstrationSubscriptionWebhookController;
...
import dev.audiobook.platform.identity.signin.BrokerAuthorizationRequestResolver;
import dev.audiobook.platform.identity.signin.BrokerIdentity;
import dev.audiobook.platform.identity.signin.OidcLoginSuccessHandler;
import dev.audiobook.platform.identity.signin.service.BrokerOidcUserService;
...
public class PlatformSecurityConfiguration {
    ...
    SessionLifecycleFilter sessionLifecycleFilter(
            IdentitySecurityProperties properties, Clock identityClock) {
        return new SessionLifecycleFilter(
                properties.sessionAbsoluteTimeout(),
                properties.sessionRotationInterval(),
                identityClock);
    }
                        ...
                        authorize ->
                                authorize
                                        .requestMatchers(
                                                ...
                                                "/actuator/health/**")
                                        .permitAll()
                                        .requestMatchers(uploadCapability)
                                        .permitAll()
                                        .requestMatchers(inspectionDelivery)
                                        .permitAll()
                                        .requestMatchers(stripeEvents)
                                        .permitAll()
                                        .requestMatchers("/api/v1/operator/action-queue/**")
                                        .hasAnyRole(
                                                "SUPPORT",
                                                "RELIABILITY",
                                                "ENTITLEMENT",
                ...
                .csrf(
                        csrf ->
                                csrf.ignoringRequestMatchers(
                                        uploadCapability, inspectionDelivery, stripeEvents))
                .oauth2Login(
                        oauth ->
