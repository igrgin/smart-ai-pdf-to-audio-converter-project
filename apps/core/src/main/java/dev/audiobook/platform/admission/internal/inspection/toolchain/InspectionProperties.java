package dev.audiobook.platform.admission.internal.inspection.toolchain;
...
public record InspectionProperties(
    ...
    public InspectionProperties {
        ...
        if (commandTimeout == null || commandTimeout.isNegative() || commandTimeout.isZero()
                || runtime == null || runtime.isNegative() || runtime.isZero()) {
            throw new IllegalArgumentException("Inspection time limits must be positive");
        }
        if (scratchPath == null || malwareCommand == null || malwareCommand.isBlank()
                || qpdfCommand == null || qpdfCommand.isBlank()) {
