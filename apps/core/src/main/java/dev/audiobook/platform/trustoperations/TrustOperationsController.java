...
public class TrustOperationsController {
    ...
    public ResponseEntity<ActionQueueView> actionQueue(Authentication authentication) {
        ...
        List<TrustOperationsService.OperationsCase> cases =
                trustOperationsService.actionQueue(StaffAuthorities.roles(authentication));
    ...
    public ResponseEntity<TrustOperationsService.CaseDetails> caseDetails(
        ...
        return ResponseEntity.ok()
                .cacheControl(CacheControl.noStore())
                .body(
                        trustOperationsService.caseDetails(
                                StaffAuthorities.context(authentication), caseId));
    }

    @PostMapping("/{caseId}/delegated-access-requests")
    public ResponseEntity<DelegatedAccessRequestWorkflow.PendingAccessRequest>
        ...
        var pending =
                delegatedAccessRequests.request(
    ...
    public ResponseEntity<TrustOperationsService.EmergencyAccessGrant> grantEmergencyAccess(
            ...
            @RequestHeader("Idempotency-Key") String operationKey,
            Authentication authentication) {
        TrustOperationsService.EmergencyAccessGrant grant =
                trustOperationsService.grantEmergencyAccess(
    ...
    public ResponseEntity<TrustOperationsService.EmergencyAccessGrant> reviewEmergencyAccess(
            ...
            Authentication authentication) {
        TrustOperationsService.EmergencyAccessGrant grant =
                trustOperationsService.reviewEmergencyAccess(
    ...
    public ResponseEntity<TrustOperationsService.PrivilegedActionResult> performPrivilegedAction(
            ...
            @RequestHeader("Idempotency-Key") String operationKey,
            Authentication authentication) {
        TrustOperationsService.PrivilegedActionResult result =
                trustOperationsService.performPrivilegedAction(
    ...
    record EmergencyReviewRequest(
            TrustOperationsService.EmergencyReviewOutcome outcome, String reviewCode) {}
    ...
    static final class StaffAuthorities {
        ...
        static EnumSet<TrustOperationsService.StaffRole> roles(Authentication authentication) {
            EnumSet<TrustOperationsService.StaffRole> roles =
                    EnumSet.noneOf(TrustOperationsService.StaffRole.class);
            if (authentication == null) {
                return roles;
            }
            for (TrustOperationsService.StaffRole role :
                    TrustOperationsService.StaffRole.values()) {
        ...
        static TrustOperationsService.StaffContext context(Authentication authentication) {
            if (authentication == null
                    || !(authentication.getPrincipal()
                            instanceof
                            dev.audiobook.platform.identity.ListenerPrincipal principal)) {
