...
public class ListenerSupportAccessController {
    ...
    public ResponseEntity<ListenerAccessView> access(Authentication authentication) {
        ...
        TrustOperationsService.ListenerAccessSummary summary = trustOperationsService.listenerAccess(listenerId);
        return ResponseEntity.ok()
                .cacheControl(CacheControl.noStore())
                .body(new ListenerAccessView(
                        summary.grants(), summary.notifications(), delegatedAccessRequests.pending(listenerId)));
    }
    ...
    public ResponseEntity<TrustOperationsService.DelegatedAccessGrant> approve(
        ...
        TrustOperationsService.DelegatedAccessGrant grant = delegatedAccessRequests.approve(
    ...
    public ResponseEntity<TrustOperationsService.DelegatedAccessGrant> revoke(
            ...
            @RequestHeader("Idempotency-Key") String operationKey,
            Authentication authentication) {
        TrustOperationsService.DelegatedAccessGrant grant = trustOperationsService.revokeDelegatedAccess(
    ...
    private static ListenerPrincipal listener(Authentication authentication) {
        if (authentication == null || !(authentication.getPrincipal() instanceof ListenerPrincipal listener)) {
