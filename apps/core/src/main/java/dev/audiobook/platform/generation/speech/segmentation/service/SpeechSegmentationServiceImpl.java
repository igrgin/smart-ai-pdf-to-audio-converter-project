...
public class SpeechSegmentationServiceImpl implements SpeechSegmentationService {
    ...
    public Manifest segment(SegmentationRequest request) {
        ...
        for (ApprovedChapter chapter : request.chapters()) {
            ...
            for (SpokenUnit unit : chapter.units()) {
                ...
                for (int pieceIndex = 0; pieceIndex < pieces.size(); pieceIndex++) {
                    ...
                    SpeechBoundaryKind boundary =
                            pieceIndex + 1 == pieces.size()
                                    ? unit.boundaryKind()
                                    : SpeechBoundaryKind.LIMIT_CONTINUATION;
                    ordered.add(
                            new UnidentifiedSegment(
                                    chapter.ordinal(),
                                    segmentOrdinal++,
                                    text,
                                    Sha256Digest.of(text),
                                    boundary));
                }
            }
            if (segmentOrdinal == 0) {
        ...
        List<Segment> segments =
                ordered.stream().map(segment -> identify(request, segment)).toList();
    ...
    public String manifestDigest(String policyVersion, List<ManifestEntry> entries) {
        ...
        if (entries.getFirst() == null
            ...
            throw new IllegalArgumentException(
                    "Persisted segment manifest must begin at the first chapter");
        }
        ...
        for (ManifestEntry entry : entries) {
            if (entry == null
                ...
                throw new IllegalArgumentException(
                        "Persisted segment manifest is not gapless or canonical");
            }
        ...
        String segmentId =
                Sha256Digest.of(
                        request.recipeDigest()
                                ...
                                + segment.spokenTextDigest());
        String operationKey =
                "speech:" + request.conversionId() + ":" + request.recipeDigest() + ":" + segmentId;
        return new Segment(
                segmentId,
                operationKey,
                segment.chapterOrdinal(),
    ...
    private static void validate(SegmentationRequest request) {
        ...
        if (request.maximumCharacters() < 1 || request.chapters().isEmpty()) {
            throw new IllegalArgumentException(
                    "Approved speech and a positive provider limit are required");
        }
    }
