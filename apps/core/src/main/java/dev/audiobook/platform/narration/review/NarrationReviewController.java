...
import dev.audiobook.platform.narration.review.dto.ReviewRequest;
import dev.audiobook.platform.narration.review.dto.ReviewResponse;
...
public class NarrationReviewController {
    ...
    public ResponseEntity<ReviewResponse> submit(
        ...
        NarrationReviewService.ReviewResult result =
        ...
        return response.cacheControl(CacheControl.noStore())
                ...
                .body(
                        new ReviewResponse(
                                result.decisionId(), result.action(), result.conversionVersion()));
    }
    ...
    private static ReviewRequest reviewRequest(Map<String, Object> value) {
        ...
        NarrationReviewService.ReviewAction action =
                NarrationReviewService.ReviewAction.valueOf(string(value.get("action")));
        List<NarrationReviewService.SectionDecision> sections =
                list(value.get("sections")).stream()
    ...
    private static NarrationReviewService.ReviewItemDecision reviewItem(Object value) {
        ...
        return new NarrationReviewService.ReviewItemDecision(
                ...
                item.containsKey("narrationSnippet")
                        ? nullableString(item.get("narrationSnippet"))
                        : null);
    }

    private static void requireKeys(
            Map<String, Object> value, Set<String> allowed, Set<String> required) {
        if (!allowed.containsAll(value.keySet()) || !value.keySet().containsAll(required)) {
            throw new IllegalArgumentException(
                    "Narration Review contains unsupported or missing fields");
        }
    }
