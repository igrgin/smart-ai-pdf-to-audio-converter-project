...
public class InspectionOutcomeRecordingServiceImpl implements InspectionOutcomeRecordingService {
    ...
    public Inspection inspect(InspectionCommand command) {
        ...
        if (command.leaseUntil() == null
                || !command.leaseUntil().isAfter(now)
                || command.leaseUntil().isAfter(now.plus(inspectionProperties.runtime()))) {
            throw new IllegalArgumentException(
                    "Inspection lease exceeds the configured runtime limit");
        }
        ...
        if (claim.retriesExhausted()) {
            inspection =
                    PublicationInspectionService.Result.rejected("INSPECTION_RETRIES_EXHAUSTED");
        } else {
            try (var publication = objectStore.read(claim.subject().submissionId())) {
                inspection =
                        publicationInspectionService.inspect(
                                publication, claim.subject().declaredMediaType());
            } catch (IOException exception) {
                inspection =
                        PublicationInspectionService.Result.rejected(
                                "INSPECTION_DEPENDENCY_FAILED");
            }
        }
        ...
        if (completed == null) {
            throw new IllegalStateException(
                    "Inspection completion transaction did not return a result");
        }
        return completed;
    }
    ...
    private Claim claim(UUID workId, String workerId, Instant leaseUntil, String operationKey) {
        ...
        InspectionWorkflowService.Claim workflowClaim =
                inspectionWorkflowService.claim(workId, workerId, leaseUntil, operationKey);
        ...
        if (workflowClaim.status()
                == InspectionWorkflowService.ClaimStatus.LEASED_BY_ANOTHER_WORKER) {
    ...
    private Inspection completeInspection(
        ...
        InspectionOutcome outcome =
                inspection.accepted() ? InspectionOutcome.ADMITTED : InspectionOutcome.REJECTED;
        Boolean recorded =
    ...
    private record InspectionSubject(
            UUID submissionId, UUID listenerId, String declaredMediaType) {}
