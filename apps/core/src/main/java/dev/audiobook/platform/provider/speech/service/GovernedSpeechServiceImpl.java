...
public class GovernedSpeechServiceImpl implements GovernedSpeechService {
    ...
    public SpeechOutcome synthesize(SpeechCommand command) {
        ...
        long submittedUnits =
                switch (capability.inputUnit()) {
                    case "UTF8_CHARACTER" ->
                            command.canonicalText()
                                    .codePointCount(0, command.canonicalText().length());
                    case "UTF8_BYTE" ->
                            command.canonicalText().getBytes(StandardCharsets.UTF_8).length;
        ...
        ProviderSpeechAdapter adapter =
                adapters.stream()
                        ...
                        .orElseThrow(
                                () ->
                                        failed(
                                                SpeechProviderException.Code
                                                        .CONFIGURATION_UNAVAILABLE,
                                                false));
        ProviderSpeechAdapter.SpeechResult result =
                adapter.synthesize(
                        new ProviderSpeechAdapter.SpeechRequest(
                                command.operationId(),
                                capability,
                                route.providerVoice(),
                                route.nativeControls(),
                                command.canonicalText()));
        if (result == null
        ...
        jdbcTemplate.update(
                ...
                command.generationRecipeId(),
                result.providerRequestId(),
                result.actualModel(),
                result.modelEvidenceSource().name(),
                result.actualRegion(),
    ...
    private static SpeechProviderException failed(
