...
public interface TrustOperationsService {
    ...
    record RevokeDelegatedAccessCommand(
            UUID listenerId, UUID grantId, long expectedVersion, String operationKey) {}
    ...
    record ListenerNotification(
            UUID notificationId, UUID caseId, UUID grantId, String eventType, Instant createdAt) {}

    record ListenerAccessSummary(
            List<DelegatedAccessGrant> grants, List<ListenerNotification> notifications) {
    ...
    record PrivilegedActionCommand(
            StaffContext staff, UUID caseId, PrivilegedAction action, String operationKey) {}

    record PrivilegedActionResult(AuthorizedResource authorizedResource, AuditEvent auditEvent) {}
}
