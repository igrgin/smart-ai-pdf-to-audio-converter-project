...
public class PrivateAudiobookLibraryServiceImpl implements PrivateAudiobookLibraryService {
    ...
    public PlaybackManifest manifest(UUID listenerId, UUID audiobookId, UUID assetVersionId) {
        ...
        List<ManifestHeader> matches = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> new ManifestHeader(
                        resultSet.getObject("audiobook_id", UUID.class),
        ...
        String expectedManifestKey = "audiobooks/" + header.audiobookId()
                + "/assets/" + header.assetVersionId()
                + "/manifest-" + header.manifestDigest() + ".json";
        if (!expectedManifestKey.equals(header.manifestObjectKey())) {
            throw new PrivateAudiobookUnavailableException();
        }
        List<PlaybackChapter> chapters = jdbcTemplate.query(
                """
                ...
                (resultSet, row) -> {
                    ...
                    return new PlaybackChapter(
                            ...
                            parts(listenerId, header.audiobookId(), header.assetVersionId(), chapterId, ordinal));
                },
                listenerId,
    ...
    private List<PlaybackPart> parts(
            ...
            UUID chapterId,
            int chapterOrdinal) {
        List<PlaybackPart> chapterParts = jdbcTemplate.query(
                """
                ...
                (resultSet, row) -> {
                    ...
                    String expectedObjectKey = "audiobooks/" + audiobookId
                            + "/assets/" + assetVersionId
                            + "/chapters/" + chapterOrdinal
                            + "/parts/" + partOrdinal + '-' + sha256 + ".mp3";
    ...
    public MediaResponse media(
        ...
        List<StoredMedia> matches = jdbcTemplate.query(
                """
        ...
        String expectedObjectKey = "audiobooks/" + audiobookId
                + "/assets/" + assetVersionId
                + "/chapters/" + media.chapterOrdinal()
                + "/parts/" + media.partOrdinal() + '-' + media.sha256() + ".mp3";
        if (!expectedObjectKey.equals(media.objectKey())) {
            throw new PrivateAudiobookUnavailableException();
        }
        String entityTag = "sha256:" + media.sha256();
        boolean honorRange = rangeHeader != null
                && (ifRangeHeader == null || ifRangeHeader.equals(quoted(entityTag)));
        try {
            byte[] completeContent = verifiedContent(media);
            if (headRequest) {
                return new MediaResponse(media.mimeType(), entityTag, media.byteLength(), null, null, new byte[0]);
            }
            if (honorRange) {
                ...
                return new MediaResponse(
                        media.mimeType(), entityTag, media.byteLength(), range.start(), range.end(), content);
            }
            return new MediaResponse(
    ...
    public ResumePosition updatePosition(UpdatePosition command) {
        ...
        List<PositionAccess> grants = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> new PositionAccess(
                        resultSet.getLong("total_duration_ms"),
        ...
        String fingerprint = sha256((command.audiobookId() + "|"
                        + command.assetVersionId() + "|"
                        + command.positionMs() + "|"
                        + command.expectedVersion())
                .getBytes(StandardCharsets.UTF_8));
        List<StoredPositionOperation> replays = jdbcTemplate.query(
                """
                ...
                """,
                (resultSet, row) -> new StoredPositionOperation(
                        resultSet.getObject("audiobook_id", UUID.class),
                        resultSet.getObject("asset_version_id", UUID.class),
