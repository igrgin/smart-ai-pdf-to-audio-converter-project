...
final class TrustOperationsCaseProjector {

    private static final Set<PrivilegedAction> VIEW_REFERENCE = Set.of(PrivilegedAction.VIEW_RESOURCE_REFERENCE);

    private final JdbcTemplate jdbcTemplate;
    private final TrustOperationsService trustOperationsService;
    private final Clock identityClock;

    void projectAuthoritativeCases() {
    ...
    private void projectFailedStages(Instant now) {
        List<OpenCaseRequest> cases = jdbcTemplate.query(
                """
                SELECT conversion_id, listener_id
                FROM workflow.audiobook_conversion
                WHERE state = 'FAILED'
                """,
                (resultSet, row) -> new Source(
                        resultSet.getObject("conversion_id", UUID.class),
                        resultSet.getObject("listener_id", UUID.class),
                        "failed-stage:" + resultSet.getObject("conversion_id", UUID.class)))
                .stream().map(source -> new OpenCaseRequest(
                        CaseType.FAILED_STAGE, source.listenerId(), "AUDIOBOOK_CONVERSION", source.resourceId(),
                        "CONVERSION_FAILED", "AUDIOBOOK_REMAINS_UNAVAILABLE", now.plusSeconds(4 * 3600),
                        85, 90, VIEW_REFERENCE, source.correlationId())).toList();
    ...
    private void projectExpiringAccess(Instant now) {
        List<OpenCaseRequest> cases = jdbcTemplate.query(
                ...
                """,
                (resultSet, row) -> new TimedSource(
                        resultSet.getObject("resource_id", UUID.class),
                        resultSet.getObject("listener_id", UUID.class),
                        "expiring-access:" + resultSet.getObject("grant_id", UUID.class),
                        resultSet.getTimestamp("expires_at").toInstant()),
                java.sql.Timestamp.from(now), java.sql.Timestamp.from(now.plusSeconds(3600)))
                .stream().map(source -> new OpenCaseRequest(
                        CaseType.EXPIRING_ACCESS, source.listenerId(), "DELEGATED_RESOURCE", source.resourceId(),
                        "ACCESS_EXPIRING", "AUTHORIZED_SUPPORT_ACCESS_ENDS_SOON", source.occurredAt(),
                        70, 80, VIEW_REFERENCE, source.correlationId())).toList();
    ...
    private void projectEntitlementInterventions(Instant now) {
        List<OpenCaseRequest> cases = jdbcTemplate.query(
                ...
                (resultSet, row) -> new TimedSource(
                        resultSet.getObject("resource_id", UUID.class),
                        resultSet.getObject("listener_id", UUID.class),
                        "entitlement-denied:" + resultSet.getObject("event_id", UUID.class),
                        resultSet.getTimestamp("occurred_at").toInstant()),
                java.sql.Timestamp.from(now.minusSeconds(24 * 3600)))
                .stream().map(source -> new OpenCaseRequest(
                        CaseType.ENTITLEMENT_INTERVENTION, source.listenerId(), "CONVERSION_ENTITLEMENT",
                        source.resourceId(), "ENTITLEMENT_INTERVENTION_REQUIRED", "CONVERSION_CANNOT_START",
                        source.occurredAt().plusSeconds(24 * 3600), 65, 70, VIEW_REFERENCE,
                        source.correlationId())).toList();
    ...
    private void projectVoiceAvailability(Instant now) {
        List<OpenCaseRequest> cases = jdbcTemplate.query(
                ...
                """,
                (resultSet, row) -> new Source(
                        resultSet.getObject("voice_id", UUID.class), null,
                        "voice-availability:" + resultSet.getObject("voice_id", UUID.class)
                                + ":" + resultSet.getString("availability")))
                .stream().map(source -> new OpenCaseRequest(
                        CaseType.VOICE_AVAILABILITY, null, "NARRATOR_VOICE", source.resourceId(),
                        "VOICE_NOT_AVAILABLE", "LISTENERS_CANNOT_SELECT_THIS_VOICE", now.plusSeconds(12 * 3600),
                        55, 60, VIEW_REFERENCE, source.correlationId())).toList();
    ...
    private void projectServiceIncidents(Instant now) {
        List<OpenCaseRequest> cases = jdbcTemplate.query(
                ...
                (resultSet, row) -> new Source(
                        resultSet.getObject("conversion_id", UUID.class),
                        resultSet.getObject("listener_id", UUID.class),
                        "service-incident:" + resultSet.getObject("profile_id", UUID.class)
                                + ":" + resultSet.getObject("conversion_id", UUID.class)))
                .stream().map(source -> new OpenCaseRequest(
                        CaseType.SERVICE_INCIDENT, source.listenerId(), "AUDIOBOOK_CONVERSION", source.resourceId(),
                        "PROVIDER_CAPABILITY_BLOCKED", "CONVERSION_STAGES_MAY_BE_UNAVAILABLE", now.plusSeconds(900),
                        100, 100, VIEW_REFERENCE, source.correlationId())).toList();
        projectAndReconcile("service-incident:", cases, now);
    }

    private void projectAndReconcile(String correlationPrefix, List<OpenCaseRequest> cases, Instant now) {
        Set<String> activeCorrelations = cases.stream()
        ...
        cases.forEach(request -> {
            ...
            jdbcTemplate.update(
                    ...
                    """,
                    java.sql.Timestamp.from(request.deadline()), request.correlationId());
        });
        jdbcTemplate.queryForList(
                ...
                .forEach(correlation -> jdbcTemplate.update(
                        "UPDATE trust_operations.operations_case SET resolved_at = ? WHERE correlation_id = ?",
                        java.sql.Timestamp.from(now), correlation));
    }
    ...
    private record TimedSource(UUID resourceId, UUID listenerId, String correlationId, Instant occurredAt) {
    }
}
