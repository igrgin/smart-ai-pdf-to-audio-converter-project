...
public class NarrationController {
    ...
    ResponseEntity<NarrationSelectionService.ConfirmedRecipe> confirm(
        ...
        var recipe = narrationSelectionService.confirm(new NarrationSelectionService.ConfirmCommand(
                listener.listenerId(), conversionId, request.voiceId(), request.pace(), expectedVersion, operationKey));
        URI location = URI.create("/api/v1/audiobook-conversions/" + conversionId + "/generation-recipe");
    ...
    private static long parseVersion(String ifMatch) {
        if (ifMatch == null || !ifMatch.matches("\"[0-9]+\"")) {
            throw new IllegalArgumentException("If-Match must contain one quoted conversion version");
        }
