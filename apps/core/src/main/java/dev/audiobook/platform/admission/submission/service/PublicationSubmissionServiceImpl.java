package dev.audiobook.platform.admission.submission.service;
...
public class PublicationSubmissionServiceImpl implements PublicationSubmissionService {
    ...
    public Creation create(CreateCommand command) {
        ...
        audit(
                request.listenerId(),
                submissionId,
                "PUBLICATION_SUBMISSION_CREATED",
                "AWAITING_UPLOAD",
                null,
                now);
        return new Creation(
    ...
    public UploadProgress upload(UploadCommand command) {
        ...
        jdbcTemplate.update(
                ...
                databaseTime(now));
        jdbcTemplate.update(
                "UPDATE upload_session SET next_offset = ?, storage_generation = ? WHERE"
                        + " submission_id = ?",
                nextOffset,
                stored.generation(),
                command.submissionId());
    ...
    public Submission confirm(ConfirmCommand command) {
        ...
        boolean matches =
                Objects.equals(command.storageGeneration(), stored.storageGeneration())
    ...
    public Submission submission(UUID listenerId, UUID submissionId) {
        ...
        return new Submission(
                ...
                stored.state(),
                stored.reasonCode(),
                stored.admittedConversionId());
    }

    private boolean applyInspectionResult(UUID workId) {
        StoredInspectionResult result =
                jdbcTemplate.queryForObject(
                        ...
                        """,
                        (resultSet, row) ->
                                new StoredInspectionResult(
                                        resultSet.getObject("submission_id", UUID.class),
                                        InspectionOutcomeRecordingService.InspectionOutcome.valueOf(
    ...
    private Submission rejectUpload(
        ...
        return terminate(
                stored, SubmissionState.REJECTED, reason, operationKey, "CONFIRM", fingerprint);
    }
    ...
    private Submission terminate(
        ...
        audit(
                stored.listenerId(),
                stored.submissionId(),
                "PUBLICATION_SUBMISSION_TERMINATED",
                state.name(),
                reason,
                now);
    ...
    private StoredChunk findChunk(UUID submissionId, long offset) {
        return jdbcTemplate.query(
                "SELECT byte_length, sha256 FROM upload_chunk WHERE submission_id = ? AND"
                        + " chunk_offset = ?",
                resultSet ->
                        resultSet.next()
                                ? new StoredChunk(
    ...
    private String capability(UUID sessionId, UUID submissionId, Instant expiresAt) {
        ...
        try {
            ...
            mac.init(
                    new SecretKeySpec(
                            properties.uploadTokenSecret().getBytes(StandardCharsets.UTF_8),
                            "HmacSHA256"));
            String signature =
    ...
    private ValidatedCreate validate(CreateCommand command) {
        ...
        String mediaType =
                requiredReference(command.mediaType(), "mediaType").toLowerCase(Locale.ROOT);
        if (!EPUB_MEDIA_TYPE.equals(mediaType) && !PDF_MEDIA_TYPE.equals(mediaType)) {
            throw new IllegalArgumentException(
                    "Only application/pdf and application/epub+zip are accepted");
        }
        if (command.byteLength() <= 0 || command.byteLength() > properties.maximumUploadBytes()) {
            throw new IllegalArgumentException(
                    "Publication byte length is outside the allowed range");
        }
        ...
        if (!properties.rightsTermsVersion().equals(terms)
                || !properties.rightsNoticeVersion().equals(notice)) {
