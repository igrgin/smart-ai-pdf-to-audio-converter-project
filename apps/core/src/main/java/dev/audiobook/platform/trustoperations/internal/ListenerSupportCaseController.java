...
public class ListenerSupportCaseController {
    ...
    public ResponseEntity<TrustOperationsService.OperationsCase> requestSupport(
        ...
        Integer owned = jdbcTemplate.queryForObject(
                "SELECT count(*) FROM library.private_audiobook WHERE audiobook_id = ? AND listener_id = ?",
                Integer.class,
                request.audiobookId(),
                listener.listenerId());
        ...
        var operationsCase = trustOperationsService.openCase(new OpenCaseRequest(
                CaseType.SUPPORT,
                listener.listenerId(),
                "PRIVATE_AUDIOBOOK",
                request.audiobookId(),
        ...
        return ResponseEntity.status(HttpStatus.CREATED).cacheControl(CacheControl.noStore()).body(operationsCase);
    }
