...
import dev.audiobook.platform.offline.signing.OfflineAuthorizationSigner;
...
public class OfflineAccessServiceImpl implements OfflineAccessService {
    ...
    public OfflineCopyAuthorization issue(IssueAuthorization command) {
        ...
        if (!existing.isEmpty()) {
            ...
            AuthorizationClaims claims =
                    claims(
                            command,
                            operation.authorizationGeneration(),
                            operation.issuedAt(),
                            operation.expiresAt());
    ...
    private OfflineManifest offlineManifest(
            UUID listenerId, PrivateAudiobookLibraryService.PlaybackManifest playback) {
        ...
        for (PrivateAudiobookLibraryService.PlaybackChapter chapter : playback.chapters()) {
            ...
            for (PrivateAudiobookLibraryService.PlaybackPart part : chapter.parts()) {
                PrivateAudiobookLibraryService.MediaResponse media =
                        libraryService.media(
                                listenerId,
                                ...
                                null,
                                false);
                if (media.totalLength() != part.byteLength()
                        || media.content().length != part.byteLength()) {
                    throw new IllegalStateException(
                            "Offline Copy source media length is inconsistent");
                }
    ...
    private List<OfflineChunk> chunks(byte[] content) {
        ...
        for (int start = 0; start < content.length; start += properties.chunkBytes()) {
            ...
            chunks.add(
                    new OfflineChunk(
                            ordinal++, start, endExclusive - 1L, chunk.length, sha256(chunk)));
        }
        return chunks;
    }
    ...
    private static AuthorizationClaims claims(
            IssueAuthorization command, long generation, Instant issuedAt, Instant expiresAt) {
