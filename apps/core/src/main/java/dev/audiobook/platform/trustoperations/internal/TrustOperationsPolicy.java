...
final class TrustOperationsPolicy {
    ...
    void requireValidCase(OpenCaseRequest request) {
    ...
    void requireDelegatedApproval(
        ...
        if (command.purposeCode().isBlank()
                ...
                || command.expiresAt().isAfter(now.plus(properties.delegatedAccessMaximumDuration()))) {
            throw new IllegalArgumentException("Invalid delegated access grant");
        }
    }

    void requireEmergencyAccess(
        ...
        if (blank(command.incidentReference())
                ...
                || !caseActions.containsAll(command.allowedActions())
                || !command.expiresAt().isAfter(now)
                || command.expiresAt().isAfter(now.plus(properties.emergencyAccessMaximumDuration()))) {
            throw new IllegalArgumentException("Invalid Emergency Access grant");
        }
    }

    Instant emergencyReviewDueAt(Instant validFrom) {
        return validFrom.plus(properties.emergencyReviewDeadline());
    }

    void requireEmergencyReview(ReviewEmergencyAccessCommand command, UUID responderId) {
    ...
    void requireEmergencyReviewOpen(Instant reviewedAt) {
        if (reviewedAt != null) {
