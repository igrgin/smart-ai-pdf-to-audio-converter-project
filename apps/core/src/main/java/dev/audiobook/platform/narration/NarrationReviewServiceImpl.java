...
public class NarrationReviewServiceImpl implements NarrationReviewService {
    ...
    public ReviewResult submit(ReviewCommand command) {
        ...
        if (conversion.version() != command.expectedConversionVersion()) {
            throw rejected(NarrationReviewRejectionReason.CONVERSION_VERSION_MISMATCH, conversion.version());
        }
        ...
        NarrationPlanService.PlanView plan = narrationPlanService.plan(command.listenerId(), command.conversionId());
        List<SectionDecision> frozenSections = command.action() == ReviewAction.SKIP_OPTIONAL
                ? recommendedSections(plan)
                : validatedSubmittedSections(plan, command.sections());
        byte[] frozenAsset = serialize(new FrozenReview(SCHEMA_VERSION, command.action(), frozenSections));
        ...
        int reviewItemCount = frozenSections.stream().mapToInt(section -> section.reviewItems().size()).sum();
        jdbcTemplate.update(
                """
                ...
                resultVersion,
                now);
        String reasonCode = command.action() == ReviewAction.SKIP_OPTIONAL
        ...
        int updated = jdbcTemplate.update(
                ...
                conversion.version());
        if (updated != 1) {
            throw rejected(NarrationReviewRejectionReason.CONVERSION_VERSION_MISMATCH, currentVersion(command));
        }
        jdbcTemplate.update(
                """
    ...
    private List<SectionDecision> recommendedSections(NarrationPlanService.PlanView plan) {
        return plan.chapters().stream()
                .map(chapter -> new SectionDecision(
                        "section-" + chapter.ordinal(),
                        sectionTitle(chapter),
                        false,
                        List.of(chapter.ordinal()),
                        chapter.reviewItems().stream()
                                .map(item -> new ReviewItemDecision(
                                        chapter.ordinal(),
                                        item.ordinal(),
                                        Treatment.valueOf(item.recommendedTreatment()),
    ...
    private List<SectionDecision> validatedSubmittedSections(
        ...
        Set<Integer> sourceChapters = plan.chapters().stream()
        ...
        for (SectionDecision section : sections) {
            ...
            for (ReviewItemDecision item : section.reviewItems()) {
                if (item == null
                        || item.treatment() == null
                        || item.narrationSnippet() != null && item.narrationSnippet().length() > MAX_SNIPPET_LENGTH) {
                    throw rejected(NarrationReviewRejectionReason.INVALID_REVIEW);
                }
    ...
    private byte[] serialize(Object value) {
        try {
            ...
            throw new IllegalStateException("Narration Review schema serialization failed", exception);
        }
    }
    ...
    private static NarrationReviewRejectedException rejected(NarrationReviewRejectionReason reason) {
