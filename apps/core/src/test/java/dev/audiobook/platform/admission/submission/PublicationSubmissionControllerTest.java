...
import dev.audiobook.platform.admission.inspection.intake.InspectionWorkDeliveryController;
...
class PublicationSubmissionControllerTest {
    ...
    private static final UUID SUBMISSION_ID =
            UUID.fromString("01985f42-5f8d-7000-8000-000000000123");

    @MockitoBean private PublicationSubmissionService submissionService;

    @MockitoBean private InspectionOutcomeRecordingService inspectionOutcomeRecordingService;
    ...
    @Test
    void authenticatedPubSubPushDurablyAcceptsOpaqueCoordinatesWithoutParsingInTheCore()
            throws Exception {
        ...
        String coordinates =
                "{\"messageId\":\"%s\",\"workId\":\"%s\"}".formatted(messageId, workId);
        String data =
                Base64.getEncoder().encodeToString(coordinates.getBytes(StandardCharsets.UTF_8));
        when(pubSubPushAuthenticator.authentic("signed-google-token")).thenReturn(true);

        mockMvc.perform(
                        post(InspectionWorkDeliveryController.DELIVERY_PATH)
    ...
    @Test
    void createAudiobookRecordsTheAttestationOnlyAfterAnAuthenticatedExplicitCommand()
            throws Exception {
        when(submissionService.create(any()))
                .thenReturn(
                        new PublicationSubmissionService.Creation(
                                SUBMISSION_ID,
        ...
        mockMvc.perform(
                        post("/api/v1/publication-submissions")
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        header().string(
                                        "Location",
                                        "/api/v1/publication-submissions/" + SUBMISSION_ID))
                .andExpect(jsonPath("$.state").value("AWAITING_UPLOAD"))
                .andExpect(
                        jsonPath("$.uploadSession.endpoint")
                                .value(
                                        "/api/v1/publication-submissions/"
                                                + SUBMISSION_ID
                                                + "/upload"))
    ...
    void opaqueUploadCapabilityTransfersAReplaySafeChunkWithoutAListenerSession() throws Exception {
        ...
        when(submissionService.upload(any()))
                .thenReturn(
                        new PublicationSubmissionService.UploadProgress(
                                chunk.length, true, "generation-23"));

        mockMvc.perform(
                        put("/api/v1/publication-submissions/" + SUBMISSION_ID + "/upload")
                                .header("Origin", "http://localhost:3000")
        ...
        org.assertj.core.api.Assertions.assertThat(command.getValue())
                ...
                .containsExactly(
                        ...
                        (long) chunk.length,
                        "c".repeat(64));
        org.assertj.core.api.Assertions.assertThat(command.getValue().bytes())
                .containsExactly(chunk);
    }

    @Test
    void listenerConfirmsStoredEvidenceAndCanPollTheContentFreeSubmissionState() throws Exception {
        when(submissionService.confirm(any()))
                .thenReturn(
                        new PublicationSubmissionService.Submission(
                                SUBMISSION_ID,
                                PublicationSubmissionService.SubmissionState.UPLOADED,
                                null,
                                null));
        when(submissionService.submission(LISTENER_ID, SUBMISSION_ID))
                .thenReturn(
                        new PublicationSubmissionService.Submission(
                                SUBMISSION_ID,
        ...
        mockMvc.perform(
                        post("/api/v1/publication-submissions/" + SUBMISSION_ID + "/confirm")
                                .header("Origin", "http://localhost:3000")
        ...
        mockMvc.perform(
                        get("/api/v1/publication-submissions/" + SUBMISSION_ID)
                                .with(authentication(listenerAuthentication())))
                .andExpect(status().isOk())
