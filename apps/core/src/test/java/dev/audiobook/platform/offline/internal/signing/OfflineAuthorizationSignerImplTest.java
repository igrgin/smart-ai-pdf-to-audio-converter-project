...
class OfflineAuthorizationSignerImplTest {
    ...
    void signsCanonicalBoundClaimsWithTheConfiguredNonProductionKeyPair() throws Exception {
        ...
        var signer = new OfflineAuthorizationSignerImpl(new LocalOfflineSignatureBoundary(properties));
        var claims = new OfflineAccessService.AuthorizationClaims(
                UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                7, "OFFLINE_PLAYBACK", Instant.parse("2026-08-01T12:00:00Z"),
        ...
        verifier.initVerify(KeyFactory.getInstance("EC").generatePublic(
                new X509EncodedKeySpec(Base64.getDecoder().decode(properties.signingPublicKey()))));
