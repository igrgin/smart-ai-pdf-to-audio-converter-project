package dev.audiobook.platform.workflow.conversion.controller;
...
import dev.audiobook.platform.admission.inspection.intake.PubSubPushAuthenticator;
...
class AudiobookConversionControllerTest {
    ...
    private static final UUID CONVERSION_ID =
            UUID.fromString("01985f42-5f8d-7000-8000-000000000125");

    @MockitoBean private AudiobookConversionService conversionService;

    @MockitoBean private ConversionLifecycleService lifecycleService;

    @MockitoBean private NarrationPlanService narrationPlanService;

    @MockitoBean private NarrationReviewService narrationReviewService;

    @MockitoBean private PublicationSubmissionService submissionService;

    @MockitoBean private AdmissionOutboxRelayService outboxRelayService;

    @MockitoBean private InspectionWorkPublisher inspectionWorkPublisher;

    @MockitoBean private PubSubPushAuthenticator pubSubPushAuthenticator;

    @MockitoBean private InspectionWorkflowService inspectionWorkflowService;

    @MockitoBean private ConversionEntitlementService entitlementService;
    ...
    @Test
    void listenerConditionallyPollsStableAwaitingReviewProgressWithoutNormalProse()
            throws Exception {
        when(conversionService.conversion(LISTENER_ID, CONVERSION_ID))
                .thenReturn(
                        new AudiobookConversionService.AudiobookConversion(
                                ...
                                1));
        when(narrationPlanService.plan(LISTENER_ID, CONVERSION_ID))
                .thenReturn(
                        new NarrationPlanService.PlanView(
                                List.of(
                                        new NarrationPlanService.ChapterView(
                                                0,
                                                "Evidence",
                                                ...
                                                List.of(),
                                                List.of(
                                                        new NarrationPlanService.ReviewItemView(
                                                                0,
                                                                1,
                                                                "TABLE",
                                                                new NarrationPlanService
                                                                        .ProvenanceView(
                                                                        "EPUB_XHTML",
                                                                        0,
                                                                        "OPS/chapter.xhtml",
        ...
        mockMvc.perform(
                        get("/api/v1/audiobook-conversions/" + CONVERSION_ID)
                ...
                .andExpect(
                        jsonPath("$.narrationPlan.chapters[0].reviewItems[0].sourceOrdinal")
                                .value(1))
                .andExpect(
                        jsonPath("$.narrationPlan.chapters[0].reviewItems[0].extractionConfidence")
                                .value(0.99))
                .andExpect(
                        jsonPath(
                                        "$.narrationPlan.chapters[0].reviewItems[0].classificationConfidence")
                                .value(0.98))
                .andExpect(
                        jsonPath("$.narrationPlan.chapters[0].reviewItems[0].treatmentConfidence")
                                .value(0.93))
                .andExpect(
                        content()
                                .string(
        ...
        when(narrationReviewService.submit(org.mockito.ArgumentMatchers.any()))
                .thenReturn(
                        new NarrationReviewService.ReviewResult(
                                decisionId, NarrationReviewService.ReviewAction.APPROVE, 2, false));
        mockMvc.perform(
                        post("/api/v1/audiobook-conversions/" + CONVERSION_ID + "/narration-review")
    ...
    void staleReviewReturnsAFocusedRecoverableConflictWithTheLatestVersion() throws Exception {
        when(narrationReviewService.submit(org.mockito.ArgumentMatchers.any()))
                .thenThrow(
                        new NarrationReviewRejectedException(
                                NarrationReviewRejectionReason.CONVERSION_VERSION_MISMATCH, 3L));
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.detail").value(org.hamcrest.Matchers.containsString("Reload")));
    }
    ...
    @Test
    void listenerSeesPersistedDamageGuidanceAndCanRequestAnIdempotentSafeRestart()
            throws Exception {
        var recovery =
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.recovery.listenerGuidance")
                                .value(
                                        "Use the saved recovery action after checking the source"
                                                + " copy."));
    ...
    void listenerCancelsAgainstTheSeenVersionWithoutSupplyingLedgerAmounts() throws Exception {
        when(lifecycleService.cancelListener(LISTENER_ID, CONVERSION_ID, 4, "cancel-31"))
                .thenReturn(
                        new ConversionLifecycleService.CancellationResult(
                                CONVERSION_ID,
        ...
        mockMvc.perform(
                        post("/api/v1/audiobook-conversions/" + CONVERSION_ID + "/cancellation")
    ...
    void staleResumeIsAConflict() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/v1/audiobook-conversions/" + CONVERSION_ID + "/resume")
    ...
    void listenerSeesStablePauseOwnershipAndResumesAgainstCurrentPolicy() throws Exception {
        ...
        when(lifecycleService.pauseDetails(LISTENER_ID, CONVERSION_ID))
                .thenReturn(
        ...
        when(lifecycleService.resume(
                ...
                .thenReturn(
                        new ConversionWorkflowService.StageView(
                                UUID.randomUUID(),
                                ConversionWorkflowService.Stage.NARRATION_ANALYSIS,
