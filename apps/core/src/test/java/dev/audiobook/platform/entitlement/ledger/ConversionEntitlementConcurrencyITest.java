...
class ConversionEntitlementConcurrencyITest {
    ...
    void concurrentAdmissionsReadCurrentStateAndCannotOversubscribeOneListener() throws Exception {
        ...
        try (var executor = Executors.newFixedThreadPool(2)) {
            Future<ConversionEntitlementService.AdmissionDecision> first =
                    executor.submit(() -> authorize(listenerId, "first", start));
            Future<ConversionEntitlementService.AdmissionDecision> second =
                    executor.submit(() -> authorize(listenerId, "second", start));
            start.countDown();
            decisions = List.of(first.get(10, TimeUnit.SECONDS), second.get(10, TimeUnit.SECONDS));
        }

        assertThat(decisions)
                .filteredOn(ConversionEntitlementService.AdmissionDecision::authorized)
                .hasSize(1);
        assertThat(decisions)
        ...
        ConversionEntitlementService.AdmissionDecision authorized =
                decisions.stream()
    ...
    private ConversionEntitlementService.AdmissionDecision authorize(
            UUID listenerId, String suffix, CountDownLatch start) throws InterruptedException {
        start.await(10, TimeUnit.SECONDS);
        return entitlementService.authorizeSpeech(
