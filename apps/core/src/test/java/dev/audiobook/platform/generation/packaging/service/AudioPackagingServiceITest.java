...
import dev.audiobook.platform.generation.packaging.AudioPackagingException;
...
class AudioPackagingServiceITest {
    ...
    void canonicalPcmIsOrderedWithOwnedSilenceAndReceivesOneAudiobookGainBeforeMp3Packaging()
        ...
        try (FfmpegTestToolchain toolchain = FfmpegTestToolchain.start(scratch)) {
            ...
            AudioPackagingService.PackagingResult result =
                    service.packageAudiobook(
                            new AudioPackagingService.PackagingRequest(
                                    ...
                                    List.of(
                                            new AudioPackagingService.Chapter(
                                                    ...
                                                    "First",
                                                    List.of(
                                                            new AudioPackagingService.AcceptedPcm(
                                                                    sine(3_000, 220),
                                                                    SpeechBoundaryKind.CHAPTER))),
                                            new AudioPackagingService.Chapter(
                                                    1,
                                                    "Second",
                                                    List.of(
            ...
            assertThat(result.chapters())
                    ...
                    .allSatisfy(
                            part -> {
                                ...
                                assertThat(part.byteLength())
                                        .isLessThanOrEqualTo(properties.maximumPartBytes());
                                assertThat(part.durationMs())
                                        .isLessThanOrEqualTo(
                    ...
                    .satisfiesExactly(
                            ...
                            second ->
                                    assertThat(second.startMs())
                                            .isEqualTo(result.chapters().getFirst().durationMs()));
            assertThat(
                            result.chapters().stream()
                                    .mapToLong(AudioPackagingService.PackagedChapter::durationMs)
                                    .sum())
    ...
    void staleAudioPolicyOrToolchainFailsClosed() {
        ...
        List<AudioPackagingService.Chapter> chapters =
                List.of(
                        new AudioPackagingService.Chapter(
                                0,
                                "Only",
                                List.of(
                                        new AudioPackagingService.AcceptedPcm(
                                                sine(1_000, 220), SpeechBoundaryKind.CHAPTER))));
    ...
    private static AudioGenerationProperties properties(String ffmpegCommand) {
        ...
        return new AudioGenerationProperties(
                source.workingAssetPath(),
                source.finalAssetPath(),
                source.workingBucket(),
                source.finalBucket(),
                source.maximumSegmentCharacters(),
                source.openAiApiKey(),
                ffmpegCommand,
                source.commandTimeout(),
                source.sampleRate(),
                source.bitrateKbps(),
                source.targetLufs(),
                source.truePeakCeilingDbtp(),
                source.maximumPartDuration(),
                source.maximumPartBytes(),
                source.continuationSilence(),
                source.paragraphSilence(),
                source.structuralSilence(),
                source.chapterSilence(),
    ...
    private static byte[] sine(int durationMs, double frequency) {
        ...
        for (int index = 0; index < samples; index++) {
            short sample =
                    (short) (8_000 * Math.sin(2 * Math.PI * frequency * index / SAMPLE_RATE));
