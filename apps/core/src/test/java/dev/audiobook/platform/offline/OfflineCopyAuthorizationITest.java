package dev.audiobook.platform.offline;
...
class OfflineCopyAuthorizationITest {
    ...
    void ownerReceivesVerifiableBoundAuthorizationAndDeterministicChunkManifest() throws Exception {
        PublishedAudiobook published = publish("offline-owner", "abcdefghij".getBytes(StandardCharsets.UTF_8));
        UUID installationId = UUID.randomUUID();

        var response = mockMvc.perform(authorizationRequest(published, installationId, "offline-operation-1"))
                ...
                .andExpect(jsonPath("$.authorization.claims.listenerId")
                        .value(published.listener().listenerId().toString()))
                .andExpect(jsonPath("$.authorization.claims.installationId").value(installationId.toString()))
                .andExpect(jsonPath("$.authorization.claims.audiobookId").value(published.audiobookId().toString()))
                .andExpect(jsonPath("$.authorization.claims.assetVersionId")
                        .value(published.assetVersionId().toString()))
                .andExpect(jsonPath("$.authorization.claims.authorizationGeneration").value(1))
                .andExpect(jsonPath("$.authorization.claims.purpose").value("OFFLINE_PLAYBACK"))
                .andExpect(jsonPath("$.manifest.totalBytes").value(10))
                .andExpect(jsonPath("$.manifest.parts[0].entityTag").value("sha256:" + published.mediaDigest()))
                ...
                .andExpect(jsonPath("$.manifest.parts[0].chunks[0].sha256")
        ...
        var replay = mockMvc.perform(authorizationRequest(published, installationId, "offline-operation-1"))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse();
    ...
    void revocationAdvancesTheAuthorizationGeneration() throws Exception {
        PublishedAudiobook published = publish("offline-revocation", "abcdefghij".getBytes(StandardCharsets.UTF_8));
    ...
    void connectedQuarantineAssetReplacementAndListenerBanAdvanceGenerations() throws Exception {
        PublishedAudiobook published = publish("offline-lifecycle", "abcdefghij".getBytes(StandardCharsets.UTF_8));
        UUID listenerId = published.listener().listenerId();

        mockMvc.perform(authorizationRequest(published, UUID.randomUUID(), "before-lifecycle-change"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.authorization.claims.authorizationGeneration").value(1));

        jdbcTemplate.update(
                "UPDATE library.private_audiobook SET availability = 'RIGHTS_QUARANTINED' WHERE audiobook_id = ?",
        ...
        jdbcTemplate.update(
                ...
                published.audiobookId());
        jdbcTemplate.update(
                "UPDATE library.private_audiobook SET current_asset_version_id = NULL WHERE audiobook_id = ?",
                published.audiobookId());
        assertThat(generation(listenerId, published.audiobookId())).isEqualTo(3);

        jdbcTemplate.update("UPDATE listener_identity SET access_state = 'BANNED' WHERE listener_id = ?", listenerId);
    ...
    void swappedListenerAndUnavailableAudiobookHaveTheSameNotFoundBoundary() throws Exception {
        PublishedAudiobook owner = publish("offline-denial", "abcdefghij".getBytes(StandardCharsets.UTF_8));
        PublishedAudiobook other = publish("offline-other", "klmnopqrst".getBytes(StandardCharsets.UTF_8));
        UUID installationId = UUID.randomUUID();

        var swapped = mockMvc.perform(authorizationRequest(
                        owner.audiobookId(), owner.assetVersionId(), other.listener(), installationId, "swapped"))
        ...
        jdbcTemplate.update(
                "UPDATE library.private_audiobook SET availability = 'RIGHTS_QUARANTINED' WHERE audiobook_id = ?",
                owner.audiobookId());
        var quarantined = mockMvc.perform(authorizationRequest(owner, installationId, "quarantined"))
                .andExpect(status().isNotFound())
                .andReturn().getResponse();
    ...
    private org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder authorizationRequest(
        ...
        return authorizationRequest(
                published.audiobookId(), published.assetVersionId(), published.listener(), installationId, operationKey);
    }
    ...
    private org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder authorizationRequest(
            ...
            UUID installationId,
            String operationKey) {
        return post("/api/v1/audiobooks/{audiobookId}/asset-versions/{assetVersionId}/offline-copy-authorizations",
                audiobookId, assetVersionId)
                ...
                .with(csrf())
                .with(authentication(UsernamePasswordAuthenticationToken.authenticated(listener, "session", Set.of())));
    }
    ...
    private boolean signatureIsValid(JsonNode authorization) throws Exception {
        ...
        byte[] signatureBytes = Base64.getUrlDecoder().decode(authorization.path("signature").asText());
    ...
    private long generation(UUID listenerId, UUID audiobookId) {
        return jdbcTemplate.queryForObject(
                "SELECT generation FROM offline_access.authorization_generation WHERE listener_id = ? AND audiobook_id = ?",
                Long.class,
                listenerId,
                audiobookId);
    }
    ...
    private PublishedAudiobook publish(String suffix, byte[] media) throws Exception {
        ...
        jdbcTemplate.update(
                ...
                """,
                recipeId, conversionId, listenerId,
                sha256(("recipe-" + suffix).getBytes(StandardCharsets.UTF_8)), Timestamp.from(now));
        jdbcTemplate.update(
        ...
        jdbcTemplate.update(
                "INSERT INTO library.audiobook_chapter (chapter_id, asset_version_id, listener_id, chapter_ordinal, display_title, start_ms, duration_ms) VALUES (?, ?, ?, 0, 'Offline chapter', 0, 10000)",
                chapterId, assetVersionId, listenerId);
        jdbcTemplate.update(
                """
                ...
                """,
                partId, chapterId, assetVersionId, listenerId, objectKey, media.length, mediaDigest);
