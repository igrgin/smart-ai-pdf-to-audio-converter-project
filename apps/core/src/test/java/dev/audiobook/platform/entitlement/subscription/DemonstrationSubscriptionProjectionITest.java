...
class DemonstrationSubscriptionProjectionITest {
    ...
    void verifiedPaidInvoiceProjectsExactlyOneLocalMonthlyGrant() throws Exception {
        ...
        assertThat(entitlementService.allowance(listenerId))
                ...
                .containsExactly(
                        ConversionEntitlementService.AllowanceStatus.AVAILABLE, 500_000L, 500_000L);
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT count(*) FROM stripe_demonstration_event_inbox WHERE"
                                        + " event_id IN ('evt_paid_first', 'evt_paid_duplicate')",
                                Long.class))
                .isEqualTo(2);
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT count(*) FROM demonstration_subscription_invoice_grant"
                                        + " WHERE stripe_invoice_id = 'in_paid_once'",
                                Long.class))
                .isEqualTo(1);
    }
    ...
    void paidEventCannotAuthorizeWhileProjectionIsPaused() throws Exception {
        ...
        try {
            ...
            ConversionEntitlementService.AdmissionDecision denied =
            ...
            assertThat(denied.denial())
                    .isEqualTo(ConversionEntitlementService.AdmissionDenial.NO_GRANT);
            assertThat(
                            jdbcTemplate.queryForObject(
                                    "SELECT projection_status FROM stripe_demonstration_event_inbox"
                                            + " WHERE event_id = 'evt_paid_paused'",
                                    String.class))
                    .isEqualTo("PENDING");
        } finally {
            resumeProjector();
    ...
    @Test
    void renewalCancellationRefundAndCorrectionRemainLocalAndNeverMakeUsageNegative()
            throws Exception {
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT subscription_status FROM demonstration_subscription WHERE"
                                        + " stripe_subscription_id = 'sub_clock'",
                                String.class))
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT count(*) FROM demonstration_subscription_grant_adjustment"
                                        + " WHERE adjustment_reference = 'stripe-refund:re_clock'",
                                Long.class))
                .isEqualTo(1);
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT MIN(committed_characters) FROM (SELECT SUM(committed_delta)"
                                        + " committed_characters FROM"
                                        + " character_entitlement_ledger_entry GROUP BY grant_id)"
                                        + " totals",
                                Long.class))
    ...
    void outOfOrderAndFailedRefundEvidenceCannotCorruptTheGrant() throws Exception {
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT projection_status FROM stripe_demonstration_event_inbox"
                                        + " WHERE event_id = 'evt_refund_before_invoice'",
                                String.class))
                .isEqualTo("PENDING");

        deliver(
                paidInvoiceEvent(
                        "evt_invoice_after_refund",
                        "in_after_refund",
                        "sub_after_refund",
                        outOfOrderListener,
                        now,
        ...
        deliver(
                paidInvoiceEvent(
                        "evt_failed_refund_invoice",
                        "in_failed_refund",
                        "sub_failed_refund",
                        failedRefundListener,
                        now,
                        now.minus(1, ChronoUnit.DAYS),
                        now.plus(29, ChronoUnit.DAYS)));
        deliver(
                refundEvent(
                        ...
                        now.plusSeconds(20),
                        "pending"));
        assertThat(entitlementService.allowance(failedRefundListener).availableCharacters())
                .isEqualTo(500_000);
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT projection_status FROM stripe_demonstration_event_inbox"
                                        + " WHERE event_id = 'evt_refund_pending'",
                                String.class))
                .isEqualTo("PENDING");
    ...
    private void deliver(String payload) throws Exception {
        mockMvc.perform(
                        post(WEBHOOK_PATH)
                                .header(
                                        "Stripe-Signature",
                                        StripeWebhookTestEvents.signature(payload, WEBHOOK_SECRET))
    ...
    private void pauseProjector() throws Exception {
        mockMvc.perform(
                        post("/api/v1/operator/demonstration-subscriptions/projector/pause")
    ...
    private void resumeProjector() throws Exception {
        mockMvc.perform(
                        post("/api/v1/operator/demonstration-subscriptions/projector/resume")
                                .header("Origin", "http://localhost:3000")
                                .with(authentication(operatorAuthentication()))
                                .with(csrf()))
                .andExpect(status().isNoContent());
    ...
    private static String invoiceCorrectionEvent(
            String eventId, String eventType, String invoiceId, Instant eventCreated) {
        return """
        {"id":"%s","object":"event","created":%d,"livemode":false,
