...
import dev.audiobook.platform.entitlement.subscription.stripe.service.StripeEventInboxService;
...
class StripeDemonstrationSubscriptionWebhookControllerTest {
    ...
    @MockitoBean private StripeEventInboxService inboxService;

    @MockitoBean private DemonstrationSubscriptionProjector projector;

    @MockitoBean private DemonstrationSubscriptionProjectorControlService projectorControlService;

    @Autowired private MockMvc mockMvc;

    @Test
    void acceptsAnAuthenticStripeTestModeEventWithoutAListenerSessionOrBrowserOrigin()
            throws Exception {
        String payload =
                """
        ...
        mockMvc.perform(
                        post(WEBHOOK_PATH)
                                .header(
                                        "Stripe-Signature",
                                        StripeWebhookTestEvents.signature(
                                                payload, "whsec_test_webhook"))
    ...
    void rejectsInvalidSignaturesAndLiveModeBeforeTheInbox() throws Exception {
        ...
        mockMvc.perform(
                        post(WEBHOOK_PATH)
                                .header(
                                        "Stripe-Signature",
                                        "t=" + Instant.now().getEpochSecond() + ",v1=00")
                                .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(
                        post(WEBHOOK_PATH)
                                .header(
                                        "Stripe-Signature",
                                        StripeWebhookTestEvents.signature(
                                                livePayload, "whsec_test_webhook"))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(livePayload))
                .andExpect(status().isBadRequest());
