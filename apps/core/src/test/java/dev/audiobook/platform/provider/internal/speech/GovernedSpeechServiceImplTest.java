...
class GovernedSpeechServiceImplTest {
    ...
    void resolvesOnlyTheFrozenQualifiedRouteAndReturnsMeteredEvidence() throws Exception {
        ...
        given(jdbcTemplate.query(anyString(), any(RowMapper.class), eq(recipeId)))
                .willAnswer(invocation -> {
                    ...
                    given(resultSet.getString("provider")).willReturn(capability.provider());
                    given(resultSet.getString("endpoint")).willReturn(capability.endpoint());
                    given(resultSet.getString("model_snapshot")).willReturn(capability.modelSnapshot());
                    ...
                    given(resultSet.getString("native_controls"))
                            .willReturn("{\"speed\":1.0,\"instructions\":\"Measured narration.\"}");
                    return List.of(mapper.mapRow(resultSet, 0));
                });
        given(capabilityService.qualified(
        ...
        given(adapter.synthesize(any())).willReturn(new ProviderSpeechAdapter.SpeechResult(
                "provider-request", capability.modelSnapshot(), capability.region(), "cedar",
                new byte[] {1, 2},
        ...
        GovernedSpeechService service = new GovernedSpeechServiceImpl(
                capabilityService,
                List.of(adapter),
                jdbcTemplate,
                Clock.fixed(Instant.parse("2026-08-01T12:00:00Z"), ZoneOffset.UTC));

        GovernedSpeechService.SpeechOutcome outcome = service.synthesize(
        ...
        assertThat(outcome.usage()).isEqualTo(
                new ProviderUsage("INPUT_CHARACTER", 17, "AUDIO_BYTE", 2));
    ...
    private static ProviderCapabilityService.CapabilityProfile capability() {
        return new ProviderCapabilityService.CapabilityProfile(
                UUID.randomUUID(), "openai-speech-eu-v2", "openai",
                ProviderCapabilityService.ServiceKind.SPEECH,
                "https://eu.api.openai.com/v1/audio/speech",
                "gpt-4o-mini-tts-2025-12-15", "SYNCHRONOUS", "eu",
                "openai-eu-zdr-v1",
                Set.of(ProviderCapabilityService.InputKind.CANONICAL_TEXT),
                4_096, "UTF8_CHARACTER", "REQUEST_PER_MINUTE", 500, 60,
