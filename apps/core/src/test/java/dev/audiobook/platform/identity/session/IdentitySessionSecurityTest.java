...
class IdentitySessionSecurityTest {
    ...
    private static final UUID LISTENER_ONE =
            UUID.fromString("01985f42-5f8d-7000-8000-000000000001");
    private static final UUID LISTENER_TWO =
            UUID.fromString("01985f42-5f8d-7000-8000-000000000002");
    ...
    @MockitoBean private PrivateAudiobookLibraryService privateAudiobookLibraryService;
    ...
    void applicationShellUsesThePerResponseNonceAndOnlySameOriginAssets() throws Exception {
        MvcResult first =
                mockMvc.perform(get("/"))
                        ...
                        .andReturn();
        MvcResult second = mockMvc.perform(get("/")).andExpect(status().isOk()).andReturn();
    ...
    void anonymousSessionPublishesOnlyCsrfAndStrictPerResponseHeaders() throws Exception {
        MvcResult first =
                mockMvc.perform(get("/api/v1/auth/session"))
                        ...
                        .andReturn();
        MvcResult second =
                mockMvc.perform(get("/api/v1/auth/session")).andExpect(status().isOk()).andReturn();

        assertThat(first.getResponse().getHeader("Content-Security-Policy"))
                .isNotEqualTo(second.getResponse().getHeader("Content-Security-Policy"));
        assertThat(first.getResponse().getContentAsString())
                .doesNotContain("token_type", "id_token", "access_token");
    }

    @Test
    void namedStaffSessionExposesOnlyAssignedDeskRolesWithoutAnIdentifierOrSuperAdminFlag()
            throws Exception {
        ...
        assertThat(result.getResponse().getContentAsString())
                .doesNotContain(LISTENER_ONE.toString());
    }
    ...
    void privateLibraryRequiresAListenerSessionAndNeverExposesListenerIds() throws Exception {
        ...
        when(conversionEntitlementService.allowance(LISTENER_TWO)).thenReturn(noGrantAllowance());
        mockMvc.perform(get("/api/v1/library")).andExpect(status().isUnauthorized());
        ...
        assertThat(first.getResponse().getContentAsString())
                .doesNotContain(LISTENER_ONE.toString(), LISTENER_TWO.toString());
        assertThat(second.getResponse().getContentAsString())
                .doesNotContain(LISTENER_ONE.toString(), LISTENER_TWO.toString(), "First Listener");
    }
    ...
    void privateLibraryExposesAnExplicitNarrationRechoiceWithoutProviderDetails() throws Exception {
        ...
        when(conversionEntitlementService.allowance(LISTENER_ONE)).thenReturn(noGrantAllowance());
        when(audiobookConversionService.conversions(LISTENER_ONE))
                .thenReturn(
                        List.of(
                                new AudiobookConversionService.AudiobookConversion(
                                        conversionId,
                                        AudiobookConversionService.ConversionState.PREPARING)));
        when(narrationSelectionService.narrationChoice(LISTENER_ONE, conversionId))
                .thenReturn(
        ...
        MvcResult result =
                mockMvc.perform(
                        ...
                        .andExpect(status().isOk())
                        .andExpect(
                                jsonPath("$.audiobooks[0].conversionId")
                                        .value(conversionId.toString()))
                        ...
                        .andExpect(
                                jsonPath("$.audiobooks[0].explicitNarrationChoiceRequired")
                                        .value(true))
                        .andReturn();

        assertThat(result.getResponse().getContentAsString())
                .doesNotContain("openai", "modelSnapshot", "providerVoice");
    }
    ...
    void privateLibraryExposesTheListenersFinalizedAudiobookForPlayback() throws Exception {
        ...
        when(audiobookConversionService.conversions(LISTENER_ONE))
                .thenReturn(
                        List.of(
                                new AudiobookConversionService.AudiobookConversion(
                                        conversionId,
                                        AudiobookConversionService.ConversionState.FINALIZED)));
        when(narrationSelectionService.narrationChoice(LISTENER_ONE, conversionId))
                .thenReturn(
                        new NarrationSelectionService.NarrationChoiceStatus(
                                3,
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.audiobooks[0].privateAudiobook.audiobookId")
                                .value(audiobookId.toString()))
                .andExpect(
                        jsonPath("$.audiobooks[0].privateAudiobook.assetVersionId")
    ...
    private static ConversionEntitlementService.Allowance noGrantAllowance() {
        return new ConversionEntitlementService.Allowance(
                ConversionEntitlementService.AllowanceStatus.NO_GRANT, 0, 0, 0, 0, "NO_GRANT");
    }
    ...
    void linkCeremonyRequiresCsrfExactOriginAndFreshCurrentAuthentication() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/links/apple").with(authentication(fresh)).with(csrf()))
                .andExpect(status().isForbidden());

        mockMvc.perform(
                        post("/api/v1/auth/links/apple")
        ...
        mockMvc.perform(
                        post("/api/v1/auth/links/facebook")
                                .header("Origin", "http://localhost:3000")
                                .with(
                                        authentication(
                                                listenerAuthentication(
    ...
    private static UsernamePasswordAuthenticationToken listenerAuthentication(
            UUID listenerId, String name, String email) {
