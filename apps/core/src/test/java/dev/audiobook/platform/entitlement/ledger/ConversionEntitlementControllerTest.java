package dev.audiobook.platform.entitlement.ledger;
...
class ConversionEntitlementControllerTest {
    ...
    void onlyAnAuthorizedOperatorCanAppendAnApprovalBackedFreeGrant() throws Exception {
        ...
        when(entitlementService.approveFreeGrant(
                        LISTENER_ID, "approval-case-22", "grant-command-22"))
                .thenReturn(
                        new ConversionEntitlementService.FreeGrant(
        ...
        when(entitlementService.approveFreeGrant(
                        LISTENER_ID, "approval-case-22", "second-grant-command"))
                .thenThrow(
                        new IllegalStateException(
                                "Listener already has a free Conversion Entitlement"));
        mockMvc.perform(
                        post(path)
                                .header("Origin", "http://localhost:3000")
    ...
    @Test
    void listenerLibraryShowsDerivedAllowanceAndClearNoGrantDenialWithoutItsIdentifier()
            throws Exception {
        when(entitlementService.allowance(LISTENER_ID))
                .thenReturn(
                        new ConversionEntitlementService.Allowance(
    ...
    @Test
    void listenerLibraryLabelsTestModeDemonstrationSubscriptionWithoutFinancialClaims()
            throws Exception {
        when(entitlementService.allowance(LISTENER_ID))
                .thenReturn(
                        new ConversionEntitlementService.Allowance(
        ...
        mockMvc.perform(get("/api/v1/library").with(authentication(listenerAuthentication())))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.conversionEntitlement.source")
                                .value("DEMONSTRATION_SUBSCRIPTION"))
                .andExpect(jsonPath("$.conversionEntitlement.demonstrationOnly").value(true))
                .andExpect(
                        jsonPath("$.conversionEntitlement.demonstrationSubscriptionStatus")
