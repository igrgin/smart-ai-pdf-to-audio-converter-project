...
class OfflineAccessControllerTest {
    ...
    private static final UUID INSTALLATION_ID = UUID.fromString("01985f42-5f8d-7000-8000-000000000133");
    private static final UUID AUDIOBOOK_ID = UUID.fromString("01985f42-5f8d-7000-8000-000000000233");
    private static final UUID ASSET_VERSION_ID = UUID.fromString("01985f42-5f8d-7000-8000-000000000333");
    private static final String PATH = "/api/v1/audiobooks/" + AUDIOBOOK_ID
            + "/asset-versions/" + ASSET_VERSION_ID + "/offline-copy-authorizations";
    ...
    void forwardsAuthenticatedConditionalMutationAndReturnsNoStoreAuthorization() throws Exception {
        ...
        OfflineAccessService.IssueAuthorization command = new OfflineAccessService.IssueAuthorization(
                LISTENER_ID, INSTALLATION_ID, AUDIOBOOK_ID, ASSET_VERSION_ID, "offline-33");
        OfflineAccessService.AuthorizationClaims claims = new OfflineAccessService.AuthorizationClaims(
                LISTENER_ID,
                INSTALLATION_ID,
                AUDIOBOOK_ID,
                ASSET_VERSION_ID,
        ...
        when(offlineAccessService.issue(command)).thenReturn(new OfflineAccessService.OfflineCopyAuthorization(
                issuedAt,
        ...
        mockMvc.perform(post(PATH)
                ...
                .andExpect(jsonPath("$.authorization.claims.listenerId").value(LISTENER_ID.toString()))
                .andExpect(jsonPath("$.manifest.assetVersionId").value(ASSET_VERSION_ID.toString()));
